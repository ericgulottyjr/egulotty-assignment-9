...
def update(frame, mlp, ax_input, ax_hidden, ax_gradient, X, y):
    ...
    current_step = frame * 10  # Since we perform 10 training steps per frame
    ...
    xx_grid, yy_grid = np.meshgrid(np.linspace(x_min, x_max, 15), np.linspace(y_min, y_max, 15))
    ...
    # Plot the transformed grid in the hidden space
    for i in range(A1_grid_x.shape[0]):
        ax_hidden.plot(
            A1_grid_x[i, :],
            A1_grid_y[i, :],
            A1_grid_z[i, :],
    ...
    for j in range(A1_grid_x.shape[1]):
        ax_hidden.plot(
            A1_grid_x[:, j],
            A1_grid_y[:, j],
            A1_grid_z[:, j],
    ...
    for (idx, neuron_idx), pos in node_positions.items():
        circle = Circle(pos, 0.035, fill=True, color='blue', zorder=2)
    ...
    ax_gradient.spines['bottom'].set_visible(False)
    ax_gradient.spines['left'].set_visible(False)
    ax_gradient.set_title(f'Network Gradients Visualization at Step {current_step}')
